...
class ExcelConnector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Local mode (offline CSV fixtures) must be explicitly requested.
        self.local_mode: bool = bool(config.get('local_mode'))

        if self.local_mode:
            # Ensure reasonable defaults in local mode
            ...
            if missing:
                raise ValueError(
                    "Missing required Excel connector credentials: " + ", ".join(missing)
                )
